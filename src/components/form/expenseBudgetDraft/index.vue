<template>
  <VCard rounded="md" class="pa-6">
    <VCardTitle class="text-h5 text-center font-weight-bold pt-0 pl-0">
      {{
        $t("form.expenseBudgetDraft.title", {
          year: selectedYears.map((y) => y + 543).join(", "),
        })
      }}
    ...
    <VCardText class="pl-0 mt-4">
      <VRow>
        <VCol class="py-1" cols="12"
          ><b>{{ $t("form.ministry") }} : </b> {{ "กระทรวง..." }}</VCol
        >
        <VCol class="py-1" cols="12"
          ><b>{{ $t("form.agency") }} : </b> {{ "หน่วนงาน..." }}</VCol
        >
      </VRow>
    </VCardText>
    ...
    <VRow class="mt-2">
      <VCol class="py-1" cols="12" md="6">
        <VSelect
          ...
          :items="years"
          :label="$t('form.expenseBudgetDraft.input.selectYear')"
    ...
    <ExpenseBudgetDraftTable :selectedYears="selectedYears" />
  </VCard>
</template>
<script setup>
...
const selectedYears = computed(() => {
  ...
  return [selectedYear.value - 1, selectedYear.value];
});
