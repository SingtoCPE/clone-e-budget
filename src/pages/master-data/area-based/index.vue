<template lang="">
  <div>
    <div class="d-flex justify-end mb-6">
      <VBtn
        color="primary"
        @click="$router.push('/master-data/area-based/create')"
      >
        {{ $t("common.btn.create") }}
      </VBtn>
    </div>
        ...
        <template #item.actions="{ item }">
          <div class="d-flex justify-space-between">
            <VIcon
              ...
              variant="text"
              class="details_icon"
              @click="$router.push(`/master-data/area-based/${item.id}`)"
            />
            <VIcon
              :icon="'tabler-edit'"
              variant="text"
              class="edit_icon"
              @click="$router.push(`/master-data/area-based/${item.id}/edit`)"
            />
